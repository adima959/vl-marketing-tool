...
import type { HistoryRecord as ActivityRecord } from '@/lib/marketing-pipeline/historyService';
...
export function ActivityFeed({ refreshKey = 0 }: ActivityFeedProps) {
  ...
  useEffect(() => {
    async function fetchActivity() {
      try {
        const response = await fetch('/api/marketing-pipeline/history?limit=15');
