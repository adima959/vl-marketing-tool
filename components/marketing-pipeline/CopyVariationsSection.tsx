...
export function CopyVariationsSection({ variations, onChange }: CopyVariationsSectionProps): React.ReactNode {
  ...
  const activeCount = local.filter(v => v.status === 'active').length;
  ...
  return (
    <div className={styles.copyVariationsSection}>
      <div className={styles.copyVariationsHeader}>
        <span className={styles.copyVariationsTitle}>Copy Variations</span>
        {activeCount > 0 && (
          <span className={styles.copyVariationsCount}>{activeCount} Active</span>
        )}
      ...
      <div className={styles.copyTableWrap}>
        {local.length === 0 ? (
          ...
          <div className={styles.copyTable}>
            ...
            {local.map((variation, idx) => (
              <div key={variation.id} className={styles.ctDataRow}>
                <div className={styles.ctRowNum}>{idx + 1}</div>
                {SECTIONS.map(section =>
                  LANGUAGES.map(lang => {
