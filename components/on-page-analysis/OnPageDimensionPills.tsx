...
function SortableTag({ dimId, onRemove, canRemove }: SortableTagProps) {
  ...
  return (
    ...
    >
      ...
      <span {...listeners}>{getOnPageDimensionLabel(dimId)}</span>
      {canRemove && (
        <CloseOutlined
...
export function OnPageDimensionPills() {
  const { dimensions, removeDimension, reorderDimensions } = useOnPageStore();
