// Marketing Pipeline History Service
...
import type { EntityType, ActivityAction } from '@/types/marketing-pipeline';
...
export async function getEntityHistory(
  ...
  const query = `
    ...
    LEFT JOIN app_pipeline_angles ang ON h.entity_type IN ('angle', 'pipeline_angle') AND h.entity_id = ang.id
    LEFT JOIN app_pipeline_messages m ON h.entity_type IN ('message', 'pipeline_message') AND h.entity_id = m.id
    LEFT JOIN app_pipeline_assets ast ON h.entity_type = 'asset' AND h.entity_id = ast.id
    LEFT JOIN app_pipeline_creatives c ON h.entity_type = 'creative' AND h.entity_id = c.id
    ...
    LEFT JOIN app_pipeline_angles old_a ON h.field_name = 'angleId' AND old_a.id::text = TRIM(BOTH '"' FROM h.old_value::text)
    LEFT JOIN app_pipeline_angles new_a ON h.field_name = 'angleId' AND new_a.id::text = TRIM(BOTH '"' FROM h.new_value::text)
