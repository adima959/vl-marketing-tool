/**
 * Verify: trial query with upsell exclusion on subscription JOIN
 * Ensures trialsApproved <= subscriptions (approval rate <= 100%)
 */
...
async function main(): Promise<void> {
  console.log('=== Verify upsell exclusion fix ===\n');

  // 1. Trial query with upsell exclusion on subscription JOIN (new fix)
  ...
  // 2. Subscription query for comparison
  ...
  // 3. Compare old (without upsell exclusion) vs new (with)
  const trialOld = await query<{ trial_count: number; trials_approved: number }>(`
    SELECT
      COUNT(DISTINCT i.id) AS trial_count,
      COUNT(DISTINCT CASE WHEN i.is_marked = 1 THEN i.id END) AS trials_approved
    FROM invoice i
  ...
  const trialNew = await query<{ trial_count: number; trials_approved: number }>(`
    SELECT
      COUNT(DISTINCT i.id) AS trial_count,
      COUNT(DISTINCT CASE WHEN i.is_marked = 1 THEN i.id END) AS trials_approved
    FROM invoice i
  ...
  console.log('\nAdwords comparison:');
  console.log(`  WITHOUT upsell excl: trials=${trialOld[0]?.trial_count}, approved=${trialOld[0]?.trials_approved}`);
  console.log(`  WITH upsell excl:    trials=${trialNew[0]?.trial_count}, approved=${trialNew[0]?.trials_approved}`);
  console.log(`  Subs:                ${subMap.get('adwords') ?? 0}`);
